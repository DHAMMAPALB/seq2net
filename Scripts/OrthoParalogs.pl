...
mkdir "$dir/OrthoParalogs";
for($i=0;$i<$n;$i++)
{
	`sed 's/"//ig' $B[$i]`;

	my @in=split(/\//,$B[$i]);$nin=@in;
	$in_id="$dir/OrthoParalogs/$in[$nin-1]";
	$in_id=~s/.faa$/.txt/ig;
	push(@ids,$in_id);
...
for($i=0;$i<$n;$i++)
{
	if($i!=$ref_n)
	{
		...
		$out_f="$dir/OrthoParalogs/$out_q[$nout_q-1]-vs-$out_s[$nout_s-1]";
		push(@fblastg,$out_f);
		`blastp -query $B[$ref_n] -subject $B[$i] -out $out_f -evalue $e_value -outfmt \"6 qseqid sseqid pident evalue bitscore\"`;
		$out_r="$dir/OrthoParalogs/$out_s[$nout_s-1]-vs-$out_q[$nout_q-1]";
		push(@bblastg,$out_r);
		...
		$out_rbbh="$dir/OrthoParalogs/RBBH.txt";
		$ngrp=0;
		for($j=0;$j<$nA3;$j++)
		{
...
open(FILE5,"$dir/OrthoParalogs/RBBH.txt");
my @A5=<FILE5>;
close FILE5;
$A5=join("",@A5);
$A5=~s/\r//ig;
...
$nn=@RV;
$out_rbbh_t="$dir/OrthoParalogs/RBBH_table.txt";
open(OUT,">$out_rbbh_t");
print OUT $RV[0];
...
$nids=@ids;
$out_ortho_t="$dir/OrthoParalogs/Orthologs_table.txt";
...
print "\nParalogs prediction ...\n";
my @out_q=split(/\//,$B[$ref_n]);$nout_q=@out_q;
$out_f="$dir/OrthoParalogs/$out_q[$nout_q-1]-vs-$out_q[$nout_q-1]";
`blastp -query $B[$ref_n] -subject $B[$ref_n] -out $out_f -evalue $e_value2 -outfmt \"6 qseqid sseqid pident evalue bitscore\"`;
open(FILE8,"$out_f");my @A8=<FILE8>;close FILE8;$nA8=@A8;
$ngrp=0;
$fig=int($nA8/100);
$fig2=$fig;
for($j=0;$j<$nA8;$j++)
{
	if($fig2 eq $i){print ".";$fig2=$fig2+$fig;}
	my @B8=split(/\t/,$A8[$j]);
	if(($B8[0] ne $B8[1]) && ($B8[2]>=$coverage2))
	{
		my @grp=grep{/$B8[1]\t$B8[0]/i}@A8;
		$ngrp=@grp;
		if($ngrp!=0)
		{
			my @C8=split(/\t/,$grp[0]);
...
@pp2=sort(@pp2);
$npp2=@pp2;
$out_rbbh_p="$dir/OrthoParalogs/RBBH_p.txt";
open(OUT,">$out_rbbh_p");
for($i=0;$i<$npp2;$i++)
{
	print OUT "$pp2[$i]\n";
...
$nnn=@RV_p;
$out_rbbh_p_t="$dir/OrthoParalogs/RBBH_p_table.txt";
open(OUT,">$out_rbbh_p_t");
print OUT "$RV_p[0]\t";
for($l=0,$k=1;$l<$nnn;$l++,$k++)
{
	if($RV_p[$l] eq $RV_p[$k]) {print OUT "$ortho_p[$l],";}
	else { print OUT "$ortho_p[$l]\n$RV_p[$k]\t";}
}
close OUT;
print "\nDone.\n";
